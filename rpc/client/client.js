 ...
 * @param server -s | --server Server (OPCIONAL)
...
program
  ...
  .action(function(env){
    var ok = true;
    env.server = env.server || require("os").hostname();
    ...
    console.log('Conectandose a ' + env.server + ':' + env.port);
    var d = dnode.connect({server: env.server, port: env.port});
